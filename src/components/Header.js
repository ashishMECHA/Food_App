...
const Title = () => {
  return (
    <Link to="/">
      <img data-testid="logo" src={logo} alt="logo" className="h-16 p-2 hover:scale-[1.1] transition-transform" />
    </Link>
  );
};
...
const NavItem = ({ svg, item }) => {
  return (
    <div className="flex gap-x-1 items-center cursor-pointer hover:">
        <p className="p-1">{svg}</p>
        {item}
    </div>
        ...
        <ul className="flex gap-x-10">
            ...
            <Link to="/cart"><li>
              <NavItem
                ...
                item="Cart"
              /><p className="absolute top-[22px] right-[8rem] text-white text-[13px]">{itemsCount}</p>
            </li></Link>
          </ul>
        </div>
